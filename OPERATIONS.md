...
- **Machine-checkable claims:** drivers carry structured fields instead of burying assertions in prose. A `direction_claim` (`up`/`down` over a `1d`/`5d`/`20d`/`60d` window) states realized price action; `numeric_claims` entries (`value`, `unit`, `refers_to`) declare every number used. Free text is display-only and may not contain numeric tokens undeclared in `numeric_claims` (whitelist: feature names, ISO dates).
...
- **API access:** the official `anthropic` Python SDK is added as a runtime dependency. Rationale: schema-constrained structured outputs (`output_config.format` guarantees the response parses against the artifact schema), typed error classes, and built-in retry/backoff for 429/5xx — hand-rolling those over stdlib `urllib` (the `notify_slack.py` precedent, which was considered) would mean more custom code maintained at 100% branch coverage for no dependency saved in practice. The API-calling boundary in `src/aims/qualitative.py` stays thin and mockable; tests never call the network.
